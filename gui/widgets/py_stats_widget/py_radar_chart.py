...
class Radar:
    ...
    def __init__(
        ...
        # Get rotation angles
        self.rotation = (2 * np.pi / self.num_labels) * np.arange(
            self.num_labels)
        ...
        mask_flip_label = (self.rotation > np.pi / 2) & (self.rotation <
                                                         np.pi / 2 * 3)
    ...
    def _setup_axis(self, facecolor="#FFFFFF", ax=None):
        ...
        ax.set_aspect("equal")
        lim = self.center_circle_radius + self.ring_width * (self.num_rings +
                                                             1.5)
    ...
    def draw_circles(self, ax=None, inner=True, **kwargs):
        ...
        if (inner
                and self.even_num_rings) or (inner is False
                                             and self.even_num_rings is False):
    ...
    def _draw_radar(self, values, ax=None, **kwargs):
        ...
        vertices = (proportion * self.num_rings *
                    self.ring_width) + self.center_circle_radius
        vertices = np.c_[self.rotation_sin * vertices,
                         self.rotation_cos * vertices]
    ...
    def draw_radar(self,
        ...
        radar, vertices = self._draw_radar(values,
                                           ax=ax,
                                           zorder=1,
                                           **kwargs_radar)
    ...
    def draw_radar_compare(self,
        ...
        if compare_values.size != self.params.size:
            raise ValueError(
                "compare_values must have the same size as params")

        radar, vertices = self._draw_radar(values, ax=ax, **kwargs_radar)
        radar2, vertices2 = self._draw_radar(compare_values,
                                             ax=ax,
                                             **kwargs_compare)
    ...
    def draw_param_labels(self, ax=None, wrap=15, offset=1, **kwargs):
        outer_ring = self.center_circle_radius + (self.ring_width *
                                                  self.num_rings)
